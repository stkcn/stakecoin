...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #StakeCoinTEST2\r");
            Send(hSocket, "WHO #StakeCoinTEST2\r");
        } else {
            // randomly join #StakeCoin00-#StakeCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #StakeCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #StakeCoin%02d\r", channel_number).c_str());
        }
